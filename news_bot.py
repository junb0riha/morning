...
[예시 문체]
"미국 증시 상승 마감. 트럼프 관세 유예 발표로 투심 급반전, S&P500 및 나스닥 기술주 중심 강한 매수세 유입. 국제유가 배럴당 97달러대로 하락하며 인플레 우려 완화. 연준 금리 동결 기조 재확인 속 위험자산 선호 심리 강화, 외국인 순매수 전환."

[작성 규칙]
- 반드시 한국어로만 작성
- 200자 이상 250자 이하 (공백 포함, 반드시 준수)
...
- 뉴스에 없는 내용 절대 추가 금지
- 아래 순서로 서술:
  1. 증시 방향 + 핵심 원인 (실제 뉴스 기반, 구체적 사건/발표/이슈 명시)
  2. 주요 매크로 변수는 변동폭이 클 때만 선택적으로 수치 포함 (금리/유가/환율/지정학 중 해당되는 것만)
  3. 투자자 심리 및 수급 흐름
...
[예시 문체]
"국내 증시 상승 마감. 미국발 관세 유예 훈풍 및 외국인 대규모 순매수 전환으로 코스피 2,500선 회복. 원달러 환율 1,420원대로 급락하며 외국인 매수 유인 확대. 반도체 업황 회복 기대감 및 기관 동반 매수로 코스닥도 강세 동참."
...
- 아래 순서로 서술:
  1. 증시 방향 + 핵심 원인 (실제 뉴스 기반, 구체적 사건/발표/이슈 명시)
  2. 주요 매크로 변수는 변동폭이 클 때만 선택적으로 수치 포함 (환율/외국인수급/유가/금리 중 해당되는 것만)
  3. 투자자 심리 및 수급 흐름
- 요약문만 출력 (제목/번호/불릿 없이 단락으로)
